...
let timeoutInterval: Double = 120
...
class ServiceHelper: NSObject {
    ...
    class func request(params: [String: Any]? = nil,
        ...
        if ConnectionManager.sharedInstance.isReachable {
            ...
            var apiUrlNew = ""
            if let urll = apiURL {
                apiUrlNew = urll.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed)!
            } else {
                return
            }
            if method == .get {
                url = URL.init(string: apiUrlNew)
            } else {
                url = requestURL(method, apiName: apiName!, parameterDict: params!)
            }
            ...
            AlertController.alert(message: NO_INTERNET_CONNECTION)
        }
    }
    ...
    class  func createRequest(url: String, headerContent: [String: String], fileName: String, httpMethod:MethodType , image: UIImage ) throws -> URLRequest {
        ...
        request.httpBody = try createBody( boundary: boundary, data: data, fileName: fileName, mimeType: mimeType, ext: data.extType)
    ...
    class  private func createBody( boundary: String, data: Data, fileName: String, mimeType: String, ext: String) throws -> Data {
        var body = Data()
        body.append("--\(boundary)\r\n")
        body.append("Content-Disposition: form-data; name=\"\(fileName)\"; filename=\"\("jpeg")\"\r\n")
    ...
    class fileprivate func requestURL(_ method: MethodType, apiName: String, parameterDict: [String: Any]) -> URL {
        ...
        let urlString = APIUrl.baseUrl + apiName
        switch method {
        case .get:
...
extension URLRequest {
    ...
    func perform(completionBlock: @escaping (AnyObject?, Error?, HTTPURLResponse?) -> Void) {
